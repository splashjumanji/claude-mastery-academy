...
export const MODULES = {
  ...
  1: {
    ...
    lessons: [
      ...
      },
      {
        ...
        displayStyle: 'grid',
        stats: [
          { label: 'Claude Opus 4', value: 'Most capable', direction: 'up', color: 'green', detail: 'Deepest reasoning, longest context, highest cost. For complex architecture and analysis.' },
          { label: 'Claude Sonnet 4', value: 'Best balance', direction: 'up', color: 'jasper', detail: 'Strong reasoning with good speed and cost. The default for most tasks.' },
          { label: 'Claude Haiku 3.5', value: 'Fastest', direction: 'neutral', color: 'amber', detail: 'Lightweight and fast. Great for high-volume tasks like classification and extraction.' },
        ],
        insight: 'Claude Code uses Sonnet by default. When you need maximum reasoning power, you can switch to Opus with /model.',
      },
      {
        id: 'l1_5',
    ...
    quiz: {
      ...
      questions: [
        ...
        },
        {
          ...
          options: ['Opus 4 (most capable)', 'Sonnet 4 (best balance)', 'Haiku 3.5 (fastest)', 'It does not matter'],
          correct: 1,
          explanation: 'Sonnet 4 is the best balance of capability, speed, and cost for daily work. Claude Code defaults to Sonnet for this reason. Opus is reserved for complex reasoning tasks; Haiku for high-volume, simpler tasks.',
        },
      ],
    },
  },
  ...
  3: {
    ...
    lessons: [
      {
        id: 'l3_1',
        heading: 'Claude Code lives in your terminal.',
        body: 'Claude Code is not a VS Code extension or a web app. It runs directly in your terminal, alongside your existing tools. It reads your codebase, understands your project structure, and edits files with your permission.',
        type: 'concept',
        keyPoints: [
          { text: 'Install: npm install -g @anthropic-ai/claude-code' },
          { text: 'Run: cd your-project && claude' },
          { text: 'It reads your project files, understands the structure, and can navigate the full codebase.' },
      ...
      },
      {
        ...
        type: 'concept',
        keyPoints: [
          { text: 'Put it at the root of your repo. Claude Code picks it up automatically.' },
          { text: 'Include: tech stack, naming conventions, testing approach, common patterns, do/don\'t rules.' },
          { text: 'Good CLAUDE.md = dramatically better code generation on the first try.' },
          { text: 'You can also place CLAUDE.md files in subdirectories for area-specific context.' },
        ],
      },
      {
        id: 'l3_4',
    ...
    lessons: [
      ...
      },
      {
        ...
        keyPoints: [
          ...
          { text: 'Keep it under 500 lines. Concise and opinionated beats comprehensive and vague.' },
        ],
      },
      {
        id: 'l4_3',
  ...
  6: {
    ...
    lessons: [
      ...
      },
      {
        ...
        body: 'Create a .claude/commands/ directory in your repo with markdown files. Each file becomes a custom slash command that Claude Code recognizes. This lets you codify common tasks as reusable prompts.',
        type: 'concept',
        keyPoints: [
          { text: 'Example: /commands/review.md containing "Review the staged changes for security issues and style violations"' },
          { text: 'Example: /commands/test.md containing "Run the test suite, analyze any failures, and fix them"' },
          { text: 'Commands can reference files, include constraints, and specify output format.' },
          { text: 'Share commands via the repo. New team members get your best practices for free.' },
        ],
      },
      {
        id: 'l6_4',
        heading: 'At Nexus: the MCP stack.',
        body: 'Nexus installs three MCP servers: GitHub (for PR management), the internal deployment API (for staging deploys), and PostgreSQL (for querying production data). Now when an engineer asks Claude Code "What is the error rate for the /auth endpoint this week?", it queries the database directly and answers with real data.',
        type: 'narrative',
      },
    ],
    quiz: {
      ...
      questions: [
        ...
        {
          id: 'q6_2',
          question: 'How do custom slash commands work in Claude Code?',
          options: [
            'They require a paid subscription to create',
            'You create markdown files in .claude/commands/ that become reusable prompts Claude Code recognizes',
            'They only work with MCP servers installed',
            'They are built into Claude Code and cannot be customized',
          ],
          correct: 1,
          explanation: 'Custom slash commands are simply markdown files in .claude/commands/. Each file becomes a command that Claude Code can invoke. They let you codify common tasks (code review, test fixing, deployment) as reusable, shareable prompts.',
        },
      ],
    },
  },
    ...
    lessons: [
      {
        ...
        keyPoints: [
          ...
          { text: 'Agents are autonomous within constraints. They adapt to unexpected results.' },
          { text: 'The Claude API\'s tool_use feature is the building block for agents.' },
        ],
      },
      {
        id: 'l8_2',
        ...
        type: 'concept',
        keyPoints: [
          { text: '1. Send a message to Claude with tool definitions.' },
          { text: '2. Check the response: does it contain a tool_use block?' },
          { text: '3. If yes: execute the tool, send the result back as a tool_result message, go to step 2.' },
          { text: '4. If no tool_use: Claude is done. Return the final text response.' },
          { text: 'This loop is the entire pattern. Everything else is tooling around it.' },
        ],
      },
      {
        id: 'l8_3',
    ...
    lessons: [
      {
        ...
        body: 'Some tasks are too complex or too broad for a single agent. Multi-agent systems decompose work across specialized Claude instances, each with its own tools and system prompt.',
        type: 'concept',
        keyPoints: [
          { text: 'Fan-out: a supervisor decomposes a task and delegates sub-tasks to worker agents in parallel.' },
          { text: 'Pipeline: output from one agent becomes input to the next. Each stage transforms the data.' },
          { text: 'Supervisor: one agent coordinates others, reviews their work, and makes final decisions.' },
          { text: 'Key trade-off: more agents = more flexible, but also more complex, slower, and more expensive.' },
        ],
      },
      {
        id: 'l11_2',
        heading: 'The fan-out pattern.',
        body: 'A supervisor agent receives a complex task, breaks it into sub-tasks, spawns multiple worker agents in parallel, collects their outputs, and synthesizes a final result. This is ideal for tasks that can be parallelized.',
        type: 'concept',
        keyPoints: [
          { text: 'Supervisor sends one API call with a decomposition prompt: "Break this into 3 sub-tasks."' },
          { text: 'Workers run in parallel: each gets a focused sub-task and specialized system prompt.' },
          { text: 'Supervisor collects worker outputs and synthesizes the final deliverable.' },
          { text: 'Example: research report → 3 workers research different subtopics → supervisor synthesizes.' },
        ],
      },
      {
        id: 'l11_3',
  ...
  12: {
    ...
    lessons: [
      ...
      },
      {
        ...
        type: 'concept',
        keyPoints: [
          { text: 'Log token counts from every API response: input_tokens, output_tokens, cache_creation_input_tokens.' },
          { text: 'Calculate cost per task: (input_tokens × input_price + output_tokens × output_price).' },
          { text: 'Use Haiku for high-volume, simpler tasks. Reserve Sonnet/Opus for complex reasoning.' },
          { text: 'Prompt caching: cache your system prompt and tool definitions to save up to 90% on input costs for repeated calls.' },
        ],
      },
      {
        id: 'l12_3',
    ...
    lessons: [
      {
        ...
        type: 'concept',
        keyPoints: [
          { text: 'Rate limiting: implement queuing and backoff. The API has per-minute and per-day limits.' },
          { text: 'Caching: use prompt caching for system prompts and tool definitions. Up to 90% input cost savings.' },
          { text: 'Batch API: for non-time-sensitive workloads, the Batch API offers 50% cost reduction.' },
      ...
      },
      {
        ...
        keyPoints: [
          ...
          { text: 'MCP ecosystem growth: hundreds of MCP servers already exist. More tools = more capable agents.' },
          { text: 'Sub-agents: Claude Code can spawn sub-agents for parallel tasks (like the fan-out pattern, built in).' },
